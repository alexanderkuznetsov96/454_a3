...
void wfModel::read( const char *filename )

{
  ...
  for (int i=0; i<vertices.size(); i++)
    vertices[i] = (objToWorldTransform * vec4( vertices[i] )).tovec3();
...

{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'n':                           /* newmtl */
      ...
      // For PREVIOUS material, check that ks + kd <= 1

      if (currentMaterial->specular[0] + currentMaterial->diffuse[0] + (1-currentMaterial->alpha) > 1 ||
          currentMaterial->specular[1] + currentMaterial->diffuse[1] + (1-currentMaterial->alpha)  > 1 ||
          currentMaterial->specular[2] + currentMaterial->diffuse[2] + (1-currentMaterial->alpha)  > 1) {
        cerr << "ERROR: Material " << currentMaterial->name << " has kd ("
             ...
             << "), and 1-alpha ("
             << 1-currentMaterial->alpha
             << " ) that sum to more than one.  Fix this."  << endl;
        exit(1);
      }
    ...
    case 'd':
      fscanf(file, "%f", &currentMaterial->alpha);
      break;

    case 'T':                   // Tr
      if (buf[1] == 'r') {
        float opacity;
        fscanf(file, "%f", &opacity );
        currentMaterial->alpha = 1.0 - opacity;
      }
      break;
...

{
  ...
  for (int i=0; i<groups.size(); i++) {
    ...
    if (numTriangles > 0) {
      ...
      //cout << "group " << thisGroup->name << ": normals " << (hasVertexNormals ? "on" : "off") << ", texture " << (hasVertexTexCoords ? "on" : "off") << endl;
...
void wfModel::draw( GPUProgram * gpuProg, mat4 &WCS_to_VCS, mat4 &VCS_to_CCS )

{
  for (int i=0; i<groups.size(); i++)
    if (groups[i]->VAOinitialized) {
      ...
      // attribute locations:
      //   0 = position
      //   1 = normal, or texcoord if normal not present
      //   2 = texcoord if normal present
...

{
  ...
  gpuProg->setInt( "texturing", (useTextures && texmap != NULL ? 1 : 0) );
}
